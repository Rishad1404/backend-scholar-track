...
const getStatusCodeFromPrismaError = (errorCode: string): number => {
    ...
    // P2025,P2001,P2018,P2015: Not Found Error
    if(["P2025","P2001","P2018","P2015"].includes(errorCode)){
    ...
    // P1010, P6010: Access denied => 403 Forbidden
    ...
    // P1008,2004,6004 : Timeout => 504 Gateway Timeout
...
export const handlePrismaClientKnownRequestError=(error: Prisma.PrismaClientKnownRequestError):TErrorResponse=>{
    ...
    // split by new line and take the first line as the error message
    const lines=cleanMessage.split("\n").filter(line=>line.trim());
    const mainMessage=lines[0] || "An unknown error occurred with database operation";

    const errorSources:TErrorSources[]=[
        {
            path:error.code,
            message:mainMessage? `${mainMessage} | ${metaInfo}`:mainMessage
        }
    ];
